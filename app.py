...
# =====================================================================
names = ['Pemeliharaan Sipil SGL'] # Nama yang akan muncul saat berhasil login
usernames = ['harsipilsgl']

# Password Asli (Teks Biasa)
passwords_asli = ['harsipilsgl_2026']

# Mengunci proses enkripsi agar password tidak ter-reset saat layar di-refresh
@st.cache_data
...
if authentication_status:
    ...
    if jenis_bangunan == "0. Pekerjaan Persiapan":
        ...
        ax.text(0.5, 0.5, 'Pekerjaan Persiapan & Umum\n(Non-Struktural)', horizontalalignment='center', verticalalignment='center', fontsize=12, fontweight='bold', color='gray')
        ax.set_axis_off()
    ...
    # =====================================================================
        ...
        panjang = st.number_input("Panjang Saluran (m)", value=100.0, key="1_p")
        t_atas = st.number_input("Tebal Atas (m)", value=0.15, key="1_ta")
        t_bawah = st.number_input("Tebal Bawah (m)", value=0.25, key="1_tb")
        t_dasar = st.number_input("Tebal Dasar (m)", value=0.30, key="1_td")
        ...
        if mode_proyek == "Bangunan Baru":
            ...
            p_bongkar = st.slider("Persen Bongkaran (%)", 0, 100, 100, key="1_sl_bongk")
            p_sedimen = st.slider("Persen Sedimen (%)", 0, 100, 30, key="1_sl_sed")
            show_bongkar = st.checkbox("Bongkaran Beton Eksisting", value=True, key="1_cb_bongk")
            h_bongkar = st.number_input("AHSP Bongkaran (Rp/m³)", value=250000, key="1_h_bongk") if show_bongkar else 0
            show_sedimen = st.checkbox("Galian Sedimen Saluran", value=True, key="1_cb_sed")
            h_sedimen = st.number_input("AHSP Galian Sedimen (Rp/m³)", value=85000, key="1_h_sed") if show_sedimen else 0
        ...
        show_bekisting = st.checkbox("Bekisting Dinding Saluran", value=True, key="1_cb_bek")
        h_bekisting = st.number_input("AHSP Bekisting (Rp/m²)", value=125000, key="1_h_bek") if show_bekisting else 0
        show_cor = st.checkbox("Pengecoran Beton Saluran", value=True, key="1_cb_cor")
        h_cor = st.number_input("AHSP Cor Beton (Rp/m³)", value=1200000, key="1_h_cor") if show_cor else 0
        show_besi = st.checkbox("Pembesian Saluran", value=True, key="1_cb_besi")
    ...
    # =====================================================================
        ...
        tebal = st.number_input("Tebal Batu (m)", value=0.25, key="2_tb")
        panjang = st.number_input("Panjang (m)", value=100.0, key="2_p")
        ...
        if mode_proyek == "Bangunan Baru":
            ...
            show_bongkar = st.checkbox("Bongkaran Batu Eksisting", value=True, key="2_cb_bongk")
            h_bongkar = st.number_input("AHSP Bongkaran (Rp/m³)", value=150000, key="2_h_bongk") if show_bongkar else 0
            if show_bongkar: item_to_add.append([f"Bongkaran Batu Eksisting ({p_bongkar}%)", vol_batu * (p_bongkar/100), "m³", h_bongkar])

        show_pasangan = st.checkbox("Pasangan Batu Kali (1:4)", value=True, key="2_cb_pas")
        h_pasangan = st.number_input("AHSP Pasangan Batu (Rp/m³)", value=950000, key="2_h_pas") if show_pasangan else 0
        show_plester = st.checkbox("Plesteran + Acian", value=True, key="2_cb_ples")
        h_plester = st.number_input("AHSP Plesteran (Rp/m²)", value=65000, key="2_h_ples") if show_plester else 0
    ...
    # =====================================================================
        ...
        if mode_proyek == "Bangunan Baru":
            ...
            show_base = st.checkbox("Lapis Pondasi Agregat A", value=True, key="3_cb_base")
            h_base = st.number_input("AHSP Agregat A (Rp/m³)", value=450000, key="3_h_base") if show_base else 0
            ...
            p_bongkar = st.slider("Persen Area Dikupas (%)", 0, 100, 100, key="3_sl_bongk")
            show_milling = st.checkbox("Cold Milling (Kupas Aspal)", value=True, key="3_cb_mill")
            h_milling = st.number_input("AHSP Milling (Rp/m³)", value=350000, key="3_h_mill") if show_milling else 0
            show_tack = st.checkbox("Lapis Perekat (Tack Coat)", value=True, key="3_cb_tack")
            h_tack = st.number_input("AHSP Tack Coat (Rp/Liter)", value=15000, key="3_h_tack") if show_tack else 0
        ...
        show_aspal = st.checkbox("Aspal Hotmix AC-WC", value=True, key="3_cb_asp")
        h_aspal = st.number_input("AHSP Aspal (Rp/m³)", value=2500000, key="3_h_asp") if show_aspal else 0
    ...
    # =====================================================================
        ...
        if mode_proyek == "Bangunan Baru":
            ...
            show_bongkar = st.checkbox("Bongkaran Rigid Eksisting", value=True, key="4_cb_bongk")
            h_bongkar = st.number_input("AHSP Bongkaran Rigid (Rp/m³)", value=450000, key="4_h_bongk") if show_bongkar else 0
            if show_bongkar: item_to_add.append([f"Bongkaran Rigid Eksisting ({p_bongkar}%)", (lebar * panjang * t_rigid) * (p_bongkar/100), "m³", h_bongkar])

        show_lc = st.checkbox("Lantai Kerja (LC)", value=True, key="4_cb_lc")
        h_lc = st.number_input("AHSP Lantai Kerja (Rp/m³)", value=950000, key="4_h_lc") if show_lc else 0
        show_bekisting = st.checkbox("Bekisting Sisi Jalan", value=True, key="4_cb_bek")
        h_bekisting = st.number_input("AHSP Bekisting (Rp/m²)", value=125000, key="4_h_bek") if show_bekisting else 0
        show_rigid = st.checkbox("Beton Rigid K-350", value=True, key="4_cb_rig")
        h_rigid = st.number_input("AHSP Beton Rigid (Rp/m³)", value=1450000, key="4_h_rig") if show_rigid else 0
        show_besi = st.checkbox("Pembesian (Dowel/Wiremesh)", value=True, key="4_cb_besi")
        r_besi = st.number_input("Rasio Besi (kg/m³)", value=60, key="4_r_besi") if show_besi else 0
    ...
    # =====================================================================
        ...
        if mode_proyek != "Bangunan Baru":
            ...
            h_bongkar = st.number_input("AHSP Bongkaran (Rp/m³)", value=350000, key="5_h_bongk") if show_bongkar else 0
            if show_bongkar: item_to_add.append([f"Bongkaran Struktur Lama ({p_bongkar}%)", vol_beton * (p_bongkar/100), "m³", h_bongkar])

        show_galian = st.checkbox("Galian Tanah Pondasi", value=True, key="5_cb_gal")
        h_galian = st.number_input("AHSP Galian (Rp/m³)", value=75000, key="5_h_gal") if show_galian else 0
        show_lc = st.checkbox("Lantai Kerja Pondasi", value=True, key="5_cb_lc")
        h_lc = st.number_input("AHSP Lantai Kerja (Rp/m³)", value=950000, key="5_h_lc") if show_lc else 0
        show_bekisting = st.checkbox("Bekisting Plat Pondasi", value=True, key="5_cb_bek")
        h_bekisting = st.number_input("AHSP Bekisting (Rp/m²)", value=145000, key="5_h_bek") if show_bekisting else 0
        show_cor = st.checkbox("Beton Plat Pondasi", value=True, key="5_cb_cor")
        h_cor = st.number_input("AHSP Beton (Rp/m³)", value=4500000, key="5_h_cor") if show_cor else 0
        show_besi = st.checkbox("Pembesian Pondasi", value=True, key="5_cb_besi")
        r_besi = st.number_input("Rasio Besi (kg/m³)", value=150, key="5_r_besi") if show_besi else 0
    ...
    # =====================================================================
        ...
        if mode_proyek == "Rehabilitasi Struktur":
            ...
            h_bongkar = st.number_input("AHSP Pembersihan (Rp/Titik)", value=500000, key="7_h_bongk") if show_bongkar else 0
            if show_bongkar: item_to_add.append([f"Pembersihan Lokasi/Bongkar Kepala ({p_bongkar}%)", jml_titik * (p_bongkar/100), "Titik", h_bongkar])

        show_bor = st.checkbox("Pengeboran Bore Pile", value=True, key="7_cb_bor")
        h_bor = st.number_input("AHSP Pengeboran (Rp/m³)", value=450000, key="7_h_bor") if show_bor else 0
        show_casing = st.checkbox("Instalasi Temporary Casing", value=True, key="7_cb_cas")
        h_casing = st.number_input("AHSP Casing (Rp/m')", value=150000, key="7_h_cas") if show_casing else 0
        show_cor = st.checkbox("Pengecoran Beton K-350", value=True, key="7_cb_cor")
        h_cor = st.number_input("AHSP Beton (Rp/m³)", value=1350000, key="7_h_cor") if show_cor else 0
        show_besi = st.checkbox("Pembesian Tulangan", value=True, key="7_cb_besi")
        r_besi = st.number_input("Rasio Besi (kg/m³)", value=180, key="7_r_besi") if show_besi else 0
    ...
    if st.session_state.rekap_proyek:
        ...
        st.info("Tabel RAB dan Rekap Data belum tersedia. Silakan tambah rincian estimasi di atas terlebih dahulu.")
